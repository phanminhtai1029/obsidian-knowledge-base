...
- DevOps: alpine image + docker network/service name (note 13 Docker — Đợt 2).

**Đã bổ sung — Đợt 4 (2026-06-25):**
- [x] **Secrets trong CI/CD** → §7 mới (3 tiểu mục: vòng đời Store→Inject→Mask→Rotate, OIDC bỏ long-lived key, secret scanning gitleaks/trufflehog) + Q&A phỏng vấn + ★ Insight + 2 câu Tự kiểm tra vào `06-DevOps/09-CI-CD-Continuous-Deployment.md` (renumber 7→8, 8→9). Bao trùm: không hardcode (Git giữ lịch sử), Docker `ARG`/`ENV` bị bake vào layer → BuildKit secret mount, PR từ fork không nhận secret, secret-in-CI ≠ secret-in-runtime. → Đây là ứng viên **cuối cùng** trong danh sách; nhóm "ứng viên chưa xử lý" nay XONG hết.
