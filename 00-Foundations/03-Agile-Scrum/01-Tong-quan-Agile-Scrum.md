...
```text
   ...
   (thấy sự thật)   (soi để phát    (điều chỉnh)
        ▲            hiện lệch)          │
        └───────────── vòng lặp ─────────┘
```
...
```text
  Sprint Planning → [ Daily Scrum × mỗi ngày, code chạy được dần ] → Sprint Review → Retrospective
  └──────────────────────── 1 Sprint (≤ 30 ngày, thường 1–2 tuần) ───────────────────────┘
        ↑ lặp lại liên tục, mỗi Sprint kết thúc bằng một Increment "có thể release"
```
