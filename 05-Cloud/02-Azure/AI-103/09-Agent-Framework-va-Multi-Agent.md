...
## 5. Năm pattern orchestration — topology từng pattern (vẽ theo sơ đồ gốc Microsoft Learn)

**Concurrent** — một input phát cho nhiều agent chạy song song, **Collector** gộp kết quả:

```mermaid
flowchart TD
    I(["Input"]) --> A1["Agent 1<br/>(Solution Generator)"] & A2["Agent 2<br/>(Solution Generator)"] & A3["Agent 3<br/>(Solution Generator)"]
    A1 & A2 & A3 --> C["Collector<br/>(gộp kết quả)"] --> R(["Result"])
```

**Sequential** — pipeline: output agent trước là input agent sau:

```mermaid
flowchart LR
    I(["Input"]) --> A1["Agent 1<br/>(Summarizer)"] --> A2["Agent 2<br/>(Translator)"] --> A3["Agent 3<br/>(QA)"] --> R(["Result"])
```

**Group chat** — mọi trao đổi đi qua **Group Chat Manager** trung tâm (request/response với từng agent và cả **Human** tham gia được):

```mermaid
flowchart TD
    I(["Input"]) --> M["Group Chat Manager"]
    H["Human"] <-->|request/response| M
    M <-->|request/response| A1["Agent 1<br/>(Marketing)"]
    M <-->|request/response| A2["Agent 2<br/>(Engineering)"]
    M <-->|request/response| A3["Agent 3<br/>(Finance)"]
    M --> R(["Result"])
```

**Handoff** — agent **chuyển quyền** theo loại việc lộ ra lúc xử lý; agent nào xong thì tự trả Result (Human có thể được hỏi xen giữa):

```mermaid
flowchart TD
    I(["Input"]) --> A["Agent A<br/>(General Support)"]
    A -->|"Handoff: Technical Issue"| B["Agent B<br/>(Technical Expert)"]
    A -->|"Handoff: Billing Issue"| C["Agent C<br/>(Billing)"]
    H["Human"] <--> A & B & C
    A & B & C -->|Complete| R(["Result"])
```

**Magentic** — phức tạp nhất, sơ đồ gốc đáng xem nguyên bản:

![[magentic-orchestration-ledgers.png]]
*Ảnh: Microsoft Learn — vòng lặp Orchestrator của Magentic với 2 sổ ghi (ledger).*

Diễn giải sơ đồ — Orchestrator quản **2 ledger** và một vòng kiểm tra:
- **Task Ledger** (kế hoạch): facts đã cho/đã xác minh, facts cần tra cứu, facts cần suy ra (tính toán/logic), **educated guesses** (phỏng đoán có cơ sở), task plan.
- **Progress Ledger** (tiến độ): task xong chưa? có **vòng lặp không hiệu quả** không? có đang tiến triển không? **ai nói tiếp theo** + chỉ dẫn cho agent đó.
- Vòng quyết định: task complete? → Yes: báo kết quả (final answer hoặc educated guess). No → còn tiến triển? → Yes: giao việc tiếp cho agent. No → tăng **stall count**; khi **stall count > 2** thì quay về **cập nhật lại Task Ledger** (lập kế hoạch lại) thay vì lặp mù.
